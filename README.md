...
The actual allocator logic itself is in `allocator.go`. The approach I use is a inspired by the "buddy system" for memory allocation. The tracking strcuture is a 32 level list, in which each level contains a list of availible subnets of that mask length (size). As pools are allocated the larger pools will be broken up and populate down the lists (from larger to smaller) and as pools are freed the pools will coalesce and move back up the lists (from smaller to larger). Additionally there is a map of allocated pools and addresses in string form to make querying for allocated resources fast.

For storage I employ a simple strategy of saving to a file on each update and loading form that file on startup. An asynchronous goroutine is responsible for saving the current state, and receives notifications via condition variable when it's time to work.
